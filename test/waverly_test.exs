defmodule WaverlyTest do
  ...
  test "error messages on missing files or non WAV files" do
    assert {:error, "test/fake_data.wav does not exist"} == Waverly.reader("test/fake_data.wav")
    assert {:error, "This library only accepts WAV files"} == Waverly.reader("README.md")
  end

  test "we can read data from a WAV file" do
    result = Waverly.reader("WAV_samples/PCM-16kHz/ex2-16kHz.wav")
    assert elem(result, 0) == :ok
    assert is_bitstring(elem(result, 1))
  end

  test "we can parse the header data from a sample 16-bit WAV file" do
    {:ok, data} = Waverly.reader("WAV_samples/PCM-16kHz/ex2-16kHz.wav")
    assert data != :nil
